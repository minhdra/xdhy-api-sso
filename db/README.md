...
| 0014 | `0014_admin_list_users_exclude_admin.sql` | Loại admin (role `sa`) khỏi `a_AdminListUsers`, `a_AdminListAppAccess` và khỏi `direct_access_count`/`eligible_user_count` của `a_AdminListApps` (`effective_access_count` giữ nguyên). Trước đó tài khoản sa hiện trong modal "Phân quyền truy cập app" dù admin bypass mọi app (tick vô tác dụng; `a_AdminAddAppAccess` lại âm thầm bỏ qua admin), và mẫu số "đủ điều kiện" phồng lên số admin. Khớp với `a_AdminListAppAccessCandidates`/`a_AdminAddAppAccess` vốn đã loại admin. Đã áp dụng lên DB thật 20/09/2026 (sau 0013). |
